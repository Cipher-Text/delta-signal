...
import { routes, type Alert, type CitizenReport, type CurrentWeatherReading, type HourlyAirQualityReading, type StationFloodForecast, type WaterBodyPagedResponse, type WaterLevelStationPagedResponse } from '@delta-signal/contracts';
import PublicNav from '../../../components/public-nav';
import { apiGet } from '../../../lib/api';
import type { MapAlert, MapDistrict, MapReport, MapLayer } from '../../../components/map-client';
import MapExplorerClient from '../../../components/map-explorer-loader';
...
export default async function MapPage(props: PageProps) {
  ...
  return <><PublicNav /><main className="map-page"><div className="map-page-heading"><div><p className="eyebrow">Delta Signal explorer</p><h1>Environmental map</h1><p>Explore current conditions, hazards, nature, and verified community activity across Bangladesh.</p></div><div className="map-page-meta"><span className="tag info">{districtPoints.length} districts mapped</span><span className="map-observed-note">Observed data is labelled by source; availability varies by layer.</span></div></div><section className="map-explorer-layout" aria-label="Bangladesh environmental map explorer"><MapExplorerClient districts={districtPoints} alerts={alerts} reports={reports} weather={weatherResult.status === 'fulfilled' ? weatherResult.value : []} airQuality={airResult.status === 'fulfilled' ? airResult.value : []} waterBodies={waterResult.status === 'fulfilled' ? waterResult.value.data : []} stations={stationResult.status === 'fulfilled' ? stationResult.value.data : []} flood={floodResult.status === 'fulfilled' ? floodResult.value : []} isLive={[districtsResult, alertsResult, reportsResult].some((r) => r.status === 'fulfilled')} initialLayer={validLayer} initialDistrictId={searchParams.district} /></section><p className="map-accessibility-note">Keyboard users can move through the layer controls and selected feature links. Important alerts are also available on the <Link href="/alerts">alerts page</Link>.</p></main></>;
}
