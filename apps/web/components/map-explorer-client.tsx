...
function floodRisk(discharge: number | null, mean: number | null, p75: number | null) {
  const risk = classifyFloodRisk(discharge, mean, p75);
  const ratio = discharge != null && mean ? discharge / mean : null;
  if (risk === 'HIGH') return { label: 'High signal', ratio };
  if (risk === 'ELEVATED') return { label: 'Elevated signal', ratio };
  return ratio != null ? { label: 'Within historical range', ratio } : null;
}
...
export default function MapExplorerClient({ districts, alerts, reports, weather, airQuality, waterBodies, stations, flood, isLive, initialLayer = 'alerts', initialDistrictId }: { districts: MapDistrict[]; alerts: MapAlert[]; reports: MapReport[]; weather: CurrentWeatherReading[]; airQuality: HourlyAirQualityReading[]; waterBodies: WaterBody[]; stations: WaterLevelStation[]; flood: StationFloodForecast[]; isLive: boolean; initialLayer?: MapLayer; initialDistrictId?: string; }) {
  ...
  return <div className="map-explorer-shell">
    ...
    <div className="map-explorer-canvas"><MapContainer bounds={BD_BOUNDS} boundsOptions={{ padding: [12, 12] }} style={{ height: '100%', width: '100%' }} scrollWheelZoom attributionControl><Resize /><TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>' />
      ...
      {layer === 'flood' && flood.map((r) => { const signal = floodRisk(r.riverDischarge, r.riverDischargeMean, r.riverDischargeP75); const markerColor = signal?.label === 'High signal' ? '#dc2626' : signal?.label === 'Elevated signal' ? '#d97706' : '#0f766e'; return <CircleMarker key={r.id} center={[r.lat, r.lng]} radius={8} pathOptions={{ color: markerColor, fillColor: signal?.label === 'High signal' ? '#fee2e2' : signal?.label === 'Elevated signal' ? '#fef3c7' : '#ccfbf1', fillOpacity: .95, weight: 2 }}><Popup><div className="map-popup"><span className="popup-kicker">Forecast · OpenMeteo GloFAS</span><strong>{r.station?.name ?? 'Water-level station'}</strong><span>{r.station?.riverName ?? 'River not recorded'} · {r.station?.district?.name ?? 'District unavailable'}</span><span>{r.riverDischarge != null ? `Discharge ${r.riverDischarge.toFixed(0)} m³/s` : 'Discharge unavailable'}{signal ? ` · ${signal.label}${signal.ratio != null ? ` (${signal.ratio.toFixed(1)}× mean)` : ''}` : ''}</span><small>Forecast date {r.forecastDate} · Simulated signal, not an official flood warning.</small><Link href={`/water-bodies/stations/${r.stationId}`}>View station →</Link></div></Popup></CircleMarker>; })}
    ...
    <div className="map-freshness">{layer === 'weather' || layer === 'air-quality' ? 'Source: Open-Meteo · Observed readings' : layer === 'flood' ? 'Source: OpenMeteo GloFAS · Simulated station discharge, not an official warning' : layer === 'reports' ? 'Source: Delta Signal verified reports' : layer === 'alerts' ? 'Source: Delta Signal operational alerts' : 'Source: Delta Signal water registry'}{!isLive && ' · Some data unavailable'}</div>
  </div>;
}
