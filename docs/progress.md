...
| CI | Done | `.github/workflows/ci.yml` — 2026-08-21; updated (2026-09-02) to add `e2e` job with postgres:16 service container, `prisma migrate deploy`, and `jest --config jest.e2e.config.js --runInBand --ci`. Remote (`Cipher-Text/nature-grid`) confirmed live 2026-09-06 — CI executes on every push to `main` and on PRs. |
