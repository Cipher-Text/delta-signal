...
Current update (2026-09-19): Phase 1 reviewed social content cards are implemented. The API now exposes the `social-content` module with source-backed `SocialPostDraft` and `SocialRenderedAsset` models, immutable source snapshots, deterministic SVG rendering in 4:5 and 1:1 formats, S3/MinIO asset storage, approval/download/manual-publication lifecycle actions, six DB-backed social permissions, and seven social audit actions. The Admin Console now has `/social-content` with district/source/station selection, editable copy, render/preview, approval, download, archive, and manual publication marking. Supported types are current weather, weather forecast, river/discharge signal, environmental alert, and biodiversity observation. Automatic suggestions, scheduled rules, and Meta publishing remain out of scope.
...
| Prisma schema | Done | 35 enums, 63 models — includes water bodies, observation measurements, restoration sub-resources, AlertType/AlertArea, DatasetVersion, StationFloodForecast, WaterLevelReading, PostGIS geom on District, Company, IndustrialFacility, AuthProvider/OAuthExchangeCode (Google OAuth), and social content drafts/assets |
| Database migration | Ready | 15 migration files; latest `20260919000000_add_social_content` is ready to apply; the configured database remains at the prior migration until `pnpm db:migrate` runs |
